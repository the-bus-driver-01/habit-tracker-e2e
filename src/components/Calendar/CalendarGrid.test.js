...
describe("CalendarGrid Component", () => {
    describe("Rendering", () => {
        ...
        it("should render day cells for all weeks in the month", () => {
            const currentDate = new Date(2024, 2); // March 2024
            const { container } = render(_jsx(CalendarGrid, { selectedDate: currentDate }));
    ...
    describe("Navigation", () => {
        it("should update month when next button is clicked", () => {
            ...
            const currentDate = new Date(2024, 2); // March 2024
            render(_jsx(CalendarGrid, { selectedDate: currentDate, onMonthChange: onMonthChange }));
        ...
        it("should display the correct month after navigation", () => {
            const currentDate = new Date(2024, 2); // March 2024
            const { rerender } = render(_jsx(CalendarGrid, { selectedDate: currentDate }));
            expect(screen.getByText("March 2024")).toBeInTheDocument();
            // Simulate moving to next month
            const nextMonth = new Date(2024, 3); // April 2024
            rerender(_jsx(CalendarGrid, { selectedDate: nextMonth, onMonthChange: () => { } }));
    ...
    describe("Date Selection", () => {
        it("should call onDateChange when a day is clicked", () => {
            ...
            const currentDate = new Date(2024, 2); // March 2024
            render(_jsx(CalendarGrid, { selectedDate: currentDate, onDateChange: onDateChange }));
            // Click on a specific day (e.g., 15th)
            const dayButtons = screen.getAllByRole("button");
            const dayButton = dayButtons.find((btn) => btn.getAttribute("aria-label")?.includes("3/15/2024"));
            if (dayButton) {
        ...
        it("should highlight the selected date", () => {
            ...
            render(_jsx(CalendarGrid, { selectedDate: selectedDate }));
        ...
        it("should not allow clicking on days from other months", () => {
            const onDateChange = vi.fn();
            const currentDate = new Date(2024, 2); // March 2024
            const { container } = render(_jsx(CalendarGrid, { selectedDate: currentDate, onDateChange: onDateChange }));
    ...
    describe("Accessibility", () => {
        ...
        it("should have ARIA labels for date cells", () => {
            ...
            const buttons = screen.getAllByRole("button");
            const labeledButtons = buttons.filter((btn) => btn.getAttribute("aria-label"));
            expect(labeledButtons.length).toBeGreaterThan(0);
        });
        it("should mark selected date with aria-pressed", () => {
            const selectedDate = new Date(2024, 2, 15); // March 15, 2024
            const { container } = render(_jsx(CalendarGrid, { selectedDate: selectedDate }));
